...
public class SortingAlgorithmLectures {
    ...
    public static int[] combine(int[] lst1, int[] lst2){
        ...
        while(index0 < lst1.length && index1 < lst2.length){
            // if lst1 has greater value, add lst1's value, otherwise add lst2's
            if(lst1[index0] < lst1[index1]){
                combined[index0 + index1] = lst1[index0];
                index0++;
            }
            else{
                combined[index0 + index1] = lst1[index1];
                index1++;
            }
        }

        // add leftovers
        while(index0 < lst1.length){
            combined[index0 + index1] = lst1[index0];
            index0++;
        }
        while(index1 < lst2.length){
            combined[index0 + index1] = lst1[index1];
            index1++;
        }
        return combined;
